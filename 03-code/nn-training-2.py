...
class NeuralNetworkVisualizer:
    ...
    def setup_figure(self):
        ...
        self.ax_network.set_title('Rete Neurale\n(2→4→2)', fontsize=12, fontweight='bold')
        self.ax_network.axis('off')
        
        # Subplot per i dati
        self.ax_data = self.fig.add_subplot(gs[0, 1])
        self.ax_data.set_title('Dataset e Classificazione', fontsize=10, fontweight='bold')
        
        # Subplot per la loss
        self.ax_loss = self.fig.add_subplot(gs[1, 1])
        self.ax_loss.set_title('Loss durante Training', fontsize=10, fontweight='bold')
        self.ax_loss.set_xlabel('Epoca')
        self.ax_loss.set_ylabel('Cross-Entropy Loss')
        
        # Subplot per i pesi
        self.ax_weights = self.fig.add_subplot(gs[:, 2])
        self.ax_weights.set_title('Evoluzione Pesi', fontsize=10, fontweight='bold')
    ...
    def draw_weights_heatmap(self):
        ...
        self.ax_weights.set_title('Pesi della Rete\n(W1: righe 0-1, W2: righe 2-5)')
    ...
    def animate(self, frame):
        ...
        # Aggiungi info frame
        self.fig.suptitle(f'Training Rete Neurale - Epoca: {frame // len(self.X)}, '
...
def main():
    ...
    # Opzionale: salva come GIF (richiede pillow o imagemagick)
    # anim.save('neural_network_training.gif', writer='pillow', fps=5)
